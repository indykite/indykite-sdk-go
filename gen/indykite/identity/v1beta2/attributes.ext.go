...
func (PropertyBatchOperations) validateMeta(meta *PropertyMetadata) error {
	if meta == nil {
		return nil
	}
...
//
// It checks if the operations represent a valid collection of property updates.
func (x PropertyBatchOperations) PreValidate() ([]string, []string, error) {
	if len(x) == 0 {
	...
	for i, v := range x {
		...
		case *PropertyBatchOperation_Add:
			...
			if err := x.validateMeta(bo.Add.Meta); err != nil {
		...
		case *PropertyBatchOperation_Replace:
			...
			if err := x.validateMeta(bo.Replace.Meta); err != nil {
				return nil, nil, fmt.Errorf("%s at position: %d", err.Error(), i)
			}
