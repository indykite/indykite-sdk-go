...
) (*ingestpb.BatchUpsertNodesResponse, error) {
	req := &ingestpb.BatchUpsertNodesRequest{
		Nodes: nodes,
	}
	if err := req.Validate(); err != nil {
		return nil, errors.NewInvalidArgumentErrorWithCause(err, "unable to call BatchUpsertNodes")
	}
	ctx = insertMetadata(ctx, c.xMetadata)
	resp, err := c.client.BatchUpsertNodes(ctx, req, opts...)
